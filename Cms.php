<?php
class webproject_Model_Import_Cms extends Mage_Core_Model_Abstract
{
	private $_importPath;
	...
	public function importCms($modelString, $itemContainerNodeString, $overwrite = false)
    {
		try
		{
			...
			{
				throw new Exception(
					Mage::helper('webproject')->__("Can't get the data file: %s", $xmlPath)
                );
			}
			...
			if ($i)
			{
				Mage::getSingleton('adminhtml/session')->addSuccess(
					Mage::helper('webproject')->__('%s item(s) was(were) imported.', $i)
				);
			}
			else
			{
				Mage::getSingleton('adminhtml/session')->addNotice(
					Mage::helper('webproject')->__('No items were imported')
				);
			}
